...
func _on_continue_button_pressed() -> void:
	...
	NodeLoader.load_scenes([GAME_SCENE, SaveManager.current_slot.saves[0]._path])


func load_done(scene: String, status: ResourceLoader.ThreadLoadStatus) -> void:
	if status == ResourceLoader.THREAD_LOAD_FAILED:
		printerr("Scene loading has been failed")
		return
	if scene == GAME_SCENE:
		var sc := ResourceLoader.load_threaded_get(scene)
		SaveManager.change_to_loaded_game(sc)
